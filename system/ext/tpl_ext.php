...
function template_ext($template,$config=array())
{
    ...
    $template = preg_replace("/<(.*?)(src=|href=|value=|background=)\"(images\/|img\/|css\/|js\/|style\/)(.*?)\"(.*?)>/i",
        "<$1$2\"".__ROOT__."/<?php echo \$config['TPL_TEMPLATE_PATH']; ?>".$lang_tpl."$3$4\"$5>", $template);
    $template = preg_replace("/<(.*?)(src=|href=|value=|background=)\'(images\/|img\/|css\/|js\/|style\/)(.*?)\'(.*?)>/i",
        "<$1$2\"".__ROOT__."/<?php echo \$config['TPL_TEMPLATE_PATH']; ?>".$lang_tpl."$3$4\"$5>", $template);
    ...
    $template = preg_replace("/\{(\\$[a-z0-9_]+)\.curl\}/iU",
        "<?php echo module('label')->get_curl($1['cid']); ?>", $template);

    //替换内容超链接
    $template = preg_replace("/\{(\\$[a-z0-9_]+)\.aurl\}/iU",
        "<?php echo module('label')->get_aurl($1['aid']); ?>", $template);
    ...
    $template = preg_replace ('/<!--foreach:{(\S+)\s+(\S+)}-->/i','<?php \$i=0;if(is_array($1)) foreach($1 AS $2) { \$i++; ?>', $template);
    /*<!--foreach:{$arr $key $vo}-->替换成 <?php $vo_i=1; if (is_array($arr) foreach($arr as $key => $vo){ ?>*/
    $template = preg_replace ('/<!--foreach:{(\S+)\s+(\S+)\s+(\S+)}-->/i','<?php \$i=0;if(is_array($1)) foreach($1 AS $2 => $3) { \$i++; ?>', $template);
    /*<!--{/foreach}-->替换成 <?php } ?>*/
    $template = preg_replace ('/<!--\{\/foreach\}-->/iU', '<?php } ?>', $template);
    ...
    $template = preg_replace ( "/\{(\\$[a-z0-9_]+)\.([a-z0-9_]+)\.([a-z0-9_]+)\s+time\=\"(.*)\"\}/i", "<?php echo date('$4',$1[\'$2\'][\'$3\']); ?>", $template);

    //TAG标签
    $template = preg_replace ( "/\{(\\$[a-z0-9_]+)\.([a-z0-9_]+)\s+tag\}/iU", "<?php \$tag=explode(',', $1['$2']); foreach (\$tag as \$value) { \$tags.= '<a href=\"".__APP__."/tags/index/tag-'.\$value.'.html\" title=\"'.\$value.'\">'.\$value.'</a>,'; } echo substr(\$tags, 0,-1); ?>", $template);

    //标题样式
    $template = preg_replace("/\{(\\$[a-z0-9_]+)\.titlex\}/iU", "<?php \$titlex=$1['title']; if(!empty($1['font_color'])){ \$titlex='<font color=\"'.$1['font_color'].'\">'.\$titlex.'</font>'; } if($1['font_bold']==1){ \$titlex='<strong>'.\$titlex.'</strong>'; }  echo \$titlex; ?>",
        $template);

    //样式+字数
    $template = preg_replace("/\{(\\$[a-z0-9_]+)\.titlex\s+len\=\"(.*)\"\}/iU", "<?php \$titlex=msubstr($1['title'],0,$2); if(!empty($1['font_color'])){ \$titlex='<font color=\"'.$1['font_color'].'\">'.\$titlex.'</font>'; } if($1['font_bold']==1){ \$titlex='<strong>'.\$titlex.'</strong>'; }  echo \$titlex; ?>",
        $template);

    //字符截取
    $template = preg_replace ( "/\{(\\$[a-z0-9_]+)\.([a-z0-9_]+)\s+len\=\"(.*)\"\}/i", "<?php echo msubstr($1['$2'],0,$3); ?>", $template);
    $template = preg_replace ( "/\{(\\$[a-z0-9_]+)\.([a-z0-9_]+)\.([a-z0-9_]+)\s+len\=\"(.*)\"\}/i", "<?php echo msubstr($1[\'$2\'][\'$3\'],0,$4); ?>", $template);
    ...
    //替换万能表单循环
    $template = preg_replace('/<!--form:{(.*)}-->/i', '<?php $form=module("label")->get_form(\'$1\'); \$form_i=0; if(is_array($form)) foreach($form as $form){  \$form_i++; ?> ',
        $template);

    //替换通用循环
    $template = preg_replace('/<!--(\S+):{(.*)}-->/i', '<?php $$1=module("label")->getlist(\'$2\'); \$$1_i=0; if(is_array($$1)) foreach($$1 as $$1){  \$$1_i++; ?> ',
        $template);
