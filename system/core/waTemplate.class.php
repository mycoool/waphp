...
class waTemplate {
	...
	public function __construct($config = array()) {
		$this->config = array_merge(waConfig::get('TPL'), (array)$config);//参数配置
		$this->assign('waTemplate', $this);
		$this->_replace = array(
		...
		if ( $is_tpl && $this->config['TPL_CACHE_ON'] ) {
			...
			if ( empty($this->config['TPL_CACHE_TYPE']) ) {
				...
				if ( (!file_exists($cacheFile)) || (filemtime($tplFile) > filemtime($cacheFile)) ) {
					file_put_contents($cacheFile, "<?php if (!defined('WAWOPHP')) exit;?>" . $this->compile($tpl,true,$diy_tpl));//写入缓存
				}
				...
				static $cache = NULL;
				$cache = is_object($cache) ? $cache : new waCache($this->config, $this->config['TPL_CACHE_TYPE']);
