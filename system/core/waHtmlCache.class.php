...
class waHtmlCache {
	...
	static public function read($module='', $action=''){	
		...
		if( empty($_SERVER['QUERY_STRING']) && preg_match("#^[a-z0-9_\-\/]+\.html$#i",$url) ){
			...
			self::$cacheFile = self::$module . '/' . self::$action . '/' . md5($_SERVER['REQUEST_URI']) . waConfig::$config['APP']['HTML_CACHE_SUFFIX'];
		}
		self::$cacheFile = waConfig::$config['APP']['HTML_CACHE_PATH'] . self::$cacheFile;
		$dir = dirname( self::$cacheFile );
		if ( !is_dir($dir) ) {
			@mkdir($dir,0777,true);
		}
		
		if( isset(waConfig::$config['APP']['HTML_CACHE_RULE'][self::$module][self::$action]) ) {
			$expires = waConfig::$config['APP']['HTML_CACHE_RULE'][self::$module][self::$action];
		} else {
			$expires = waConfig::$config['APP']['HTML_CACHE_RULE'][self::$module]['*'];
		}
	...
	static private function _checkRule() {
		...
		if ( (waConfig::$config['APP']['HTML_CACHE_ON'] == false) || empty(waConfig::$config['APP']['HTML_CACHE_RULE']) ) {
			return false;
		}
		return isset(waConfig::$config['APP']['HTML_CACHE_RULE'][self::$module][self::$action]) || isset(waConfig::$config['APP']['HTML_CACHE_RULE'][self::$module]['*']);
	}
}