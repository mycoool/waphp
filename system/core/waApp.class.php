...
class waApp {
    ...
    public function __construct( $config=array() ) {
		...
		define('WA_VER', '2.0.2012.1203');//框架版本号,后两段表示发布日期
		define('WA_CORE_PATH', dirname(__FILE__) );//当前文件所在的目录
		
        require(WA_CORE_PATH . '/waConfig.class.php');//加载默认配置
		$this->appConfig = array_merge(waConfig::get('APP'), $config);//参数配置
		waConfig::set('APP', $this->appConfig );
		defined('DEBUG') or define('DEBUG', waConfig::get('DEBUG'));
		...
		require(WA_CORE_PATH . '/waError.class.php');	//加载错误处理类

		//加载常用函数库
		if ( is_file(WA_CORE_PATH . '/../lib/common.function.php') ) {
			require(WA_CORE_PATH . '/../lib/common.function.php');
		}	

		//加载扩展函数库
		if ( is_file(WA_CORE_PATH . '/../ext/extend.php') ) {
			require(WA_CORE_PATH . '/../ext/extend.php');
		}	
	}
    ...
    public function run() {
		try{
			...
			//如果存在回调函数wa_app_end，则在即将结束前调用
			if ( function_exists('wa_app_end') ) {
				wa_app_end();
			}
		} catch( Exception $e){
			waError::show( $e->getMessage() );
		}	
	}
	...
	private function _readHtmlCache($module = '', $action = '') {	
		if ( $this->appConfig['HTML_CACHE_ON'] ) {
			require_once(WA_CORE_PATH . '/waHtmlCache.class.php');
			return waHtmlCache::read($module, $action);
		}
		return false;
	}
	...
	public function autoload($classname) {   
		$dir_array = array(	$this->appConfig['MODULE_PATH'],	//模块文件
							WA_CORE_PATH . '/../lib/',	//官方扩展库
							WA_CORE_PATH . '/../ext/',	//第三方扩展库
							WA_CORE_PATH . '/',	//核心文件
