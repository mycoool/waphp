...
class waModel{
    ...
    public function __construct( $config = array() ) {
		$this->config = array_merge(waConfig::get('DB'), $config);	//参数配置
	...
	public function connect() {
		$dbDriver = 'wa' . ucfirst( $this->config['DB_TYPE'] );
    ...
    public function __call($method, $args) {
        ...
        if ( in_array($method, array('field','data','where','group','having','order','limit','cache')) ) {
			...
			throw new Exception($method . '方法在waModel.class.php类中没有定义');
		}
    }
    ...
    public function update() {
		$table = $this->options['table'];	//当前表
		$this->format_data_by_filter($this->options['data']); //过滤多余字段
	...
	public function initCache() {		
		if (is_object($this->cache)) {
			...
			require_once(dirname(__FILE__) . '/waCache.class.php');
			$this->cache = new waCache($this->config, $this->config['DB_CACHE_TYPE']);
			return true;
		} else {
			return false;
		}
...
//==================================添加内容========================================
    ...
    public function format_data_by_fill(array $data=array()){
        ...
        foreach ($data as $fieldname => $value) {
            array_key_exists($fieldname,$defaultData) && $defaultData[$fieldname]=$value;
        }
        $this->options['data']=$this->quote_array(array_filter($defaultData,array($this,'field_empty'))); 
    }

    public function field_empty($val) {
    	if (empty($val)&&$val<>'0') { 
        	return false;
    	}else{
    		return true; 
    ...
    public function format_data_by_filter(array $data=array()){
        $defaultData=$this->fields_default();
        $t=array();
        if(empty($data)) return $t;
        foreach ($defaultData as $fieldname => $value) {
            array_key_exists($fieldname,$data) && $t[$fieldname]=$data[$fieldname];
        }

        $this->options['data']=$this->quote_array($t);
    }
