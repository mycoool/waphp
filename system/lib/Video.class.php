...
class Video {
    ...
    public function getVideoInfo($link) {
        ...
        if(preg_match("/(youku.com|youtube.com|qq.com|ku6.com|sohu.com|sina.com.cn|tudou.com|yinyuetai.com|letv.com|56.com)$/i", $parseLink['host'], $hosts)) {
    ...
    public function _video_getflashinfo($link, $host) {
		...
		if('youku.com' == $host)
        {
            $v = file_get_contents('https://openapi.youku.com/v2/videos/show_basic.json?client_id=3525c762d12a375b&video_url='.urlencode($link));
            $vi = json_decode($v,true);
            //print_r($vi);
            ...
            $desc = $vi['description'];
            $youku_str = array('—优酷网，视频高清在线观看','—在线播放');
            $title[1] = str_replace($youku_str,'',$title[1]);
            if(!empty($vi['title'])){
                $title[1] =$vi['title'];
            }
        ...
        {
            preg_match("/vid:(.*),/i",$content,$flashvar);
            preg_match("/share:\{pic:\"(.*)\",url/i",$content,$img);
            preg_match("/title:\"(.*)\",/i",$content,$title);
            $flash_url = 'http://i7.imgs.letv.com/player/swfPlayer.swf?autoPlay=0&id='.$flashvar[1];
            //$base = base64_encode(file_get_contents($img[1]));
            //$img[1] = 'data:image/jpeg;base64,'.$base;
        }
        ...
        $return['host']   = str_replace(array('.com','.com.cn'),'',$host);
        return $return;
    }
