...
function  module($module){
	...
	if(!isset($config['MODULE_PATH'])){
		$config['MODULE_PATH']=waConfig::get('MODULE_PATH');
		$config['MODULE_SUFFIX']=waConfig::get('MODULE_SUFFIX');
...
if(!function_exists('model')){
	function  model($model){
		...
		if(!isset($config['MODEL_PATH'])){
			$config['MODEL_PATH']=waConfig::get('MODEL_PATH');
			$config['MODEL_SUFFIX']=waConfig::get('MODEL_SUFFIX');
...
//加密函数，可用wa_decode()函数解密，$data：待加密的字符串或数组；$key：密钥；$expire 过期时间
function wa_encode($data,$key='',$expire = 0)
{
...
//wa_encode之后的解密函数，$string待解密的字符串，$key，密钥
function wa_decode($string,$key='')
{
	$ckey_length = 4;
	$key = md5($key);
	$keya = md5(substr($key, 0, 16));
